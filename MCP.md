...
Tripp includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your Tripp administrator before it becomes available.
...
4. Your browser opens Tripp's consent screen, where you choose which scopes (permissions) to grant.
...
> **Requirement:** The `APP_URL` environment variable must be set to your Tripp instance's public URL for OAuth
...
Tripp's MCP server supports three authentication methods. OAuth 2.1 is the recommended path for all external clients.
...
| **Web session JWT** | — | Full access | Session-based | Used internally by the Tripp web UI. Not intended for external clients. |
...
When connecting via OAuth 2.1, you grant specific scopes during the consent step. Tripp registers only the MCP tools
...
| **Demo mode restrictions**              | If Tripp is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your Tripp data. MCP clients can read these to understand the current state before
making changes.
...
Tripp exposes tools organized by feature area. Use `get_trip_summary` as a starting point — it returns everything about a
...
Before writing anything to Tripp, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to Tripp:
- Create the trip
...
PDF of the generated trip: [./docs/Tripp-Generated-by-MCP.pdf](./docs/Tripp-Generated-by-MCP.pdf)
