...
beforeEach(() => {
  ...
  // GET returns AirTrail-owned detail Tripp doesn't model — must survive the writeback.
...
describe('pushReservationToAirtrail write gate (#1240)', () => {
  ...
  it('writes back, preserving AirTrail-owned fields, when the owner has opted in', async () => {
    ...
    expect(payload.from).toBe('JFK'); // Tripp-managed field still applied as a code
  });
});
