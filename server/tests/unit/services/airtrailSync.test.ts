...
/**
 * An AirTrail flight as GET returns it, including the fields Tripp doesn't model.
...
function existingFlight(
  ...
  return {
    ...
    // AirTrail-owned detail Tripp never surfaces — must survive a writeback (#1240).
...
/** A linked Tripp reservation (the shape getReservationWithJoins returns). */
...
describe('airtrailSync.buildSavePayload', () => {
  it('round-trips the AirTrail-owned fields Tripp does not model (issue #1240)', () => {
  ...
  it('writes the Tripp time to the SCHEDULED fields so it round-trips on the next pull', () => {
    // Import reads the scheduled time, so a Tripp edit must be pushed back there
  ...
  it('blanks the scheduled time when the Tripp reservation has only a date', () => {
  ...
  it('still applies the Tripp-owned edits on top of the preserved fields', () => {
    const payload = buildSavePayload(
      reservation({
        reservation_time: '2021-09-01T20:30',
        notes: 'changed in Tripp',
  ...
  it('preserves AirTrail aircraft/airline/reason when Tripp metadata omits them (#1240)', () => {
    // A Tripp edit can drop these AirTrail-owned fields from metadata; the writeback
