...
describe('FilesController (parity with the legacy /api/trips/:tripId/files route)', () => {
  ...
  describe('POST / (upload)', () => {
    ...
    it('403 in demo mode for a demo email', () => {
      ...
      expect(thrown(() => new FilesController(fsvc()).upload(user, '5', file, {}))).toEqual({ status: 403, body: { error: 'Uploads are disabled in demo mode. Self-host Tripp for full functionality.' } });
    });
