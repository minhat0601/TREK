...
// Tripp's internal language codes mostly coincide with valid BCP-47 codes, but a
...
// Operators behind locked-down egress — or running their own Overpass — can point Tripp
...
export async function resolveGoogleMapsUrl(url: string): Promise<{ lat: number; lng: number; name: string | null; address: string | null; google_ftid: string | null }> {
  ...
  if (!coords) {
    try {
      const pageRes = await followRedirects(resolvedUrl, {
        headers: { 'User-Agent': 'Tripp-Travel-Planner/1.0' },
      });
  ...
  const nominatimRes = await fetch(
    ...
    { headers: { 'User-Agent': 'Tripp-Travel-Planner/1.0' }, signal: AbortSignal.timeout(8000) }
  );
