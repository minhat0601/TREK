...
export function buildWebhookBody(url: string, payload: { event: string; title: string; body: string; tripName?: string; link?: string }): string {
  ...
  if (isDiscord) {
    return JSON.stringify({
      embeds: [{
        ...
        url: payload.link,
        color: 0x3b82f6,
        footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'Tripp' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in Tripp>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'Tripp' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'Tripp — Test Notification',
      text: 'This is a test email from Tripp. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, { event: 'test', title: 'Test Notification', body: 'This is a test webhook from Tripp. If you received this, your webhook configuration is working correctly.' });
...
export async function testNtfy(cfg: { topic: string; server?: string | null; token?: string | null }): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      ...
      body: 'This is a test notification from Tripp. If you received this, your ntfy configuration is working correctly.',
    });
