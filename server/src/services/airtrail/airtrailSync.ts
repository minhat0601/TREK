 ...
 * updated_at) and, when a flight is gone from AirTrail, keep the Tripp row but
 ...
 * GET object) and overwrite ONLY the fields Tripp manages. Everything else —
...
export function buildSavePayload(reservation: any, existing: AirtrailFlightRaw): AirtrailSavePayload | null {
  ...
  // Push the seat the user set in Tripp onto their own AirTrail seat (the one with
  ...
  // overwrite only what Tripp manages. `from`/`to`/`airline`/`aircraft` come back
  ...
  return {
    ...
    // Import reads the SCHEDULED time, so a Tripp edit must write back there too —
    ...
    // These are AirTrail-owned details Tripp doesn't surface in its edit UI — a Tripp
    // edit can leave them out of `metadata`. Preserve AirTrail's current value when
    // Tripp has none rather than nulling it out (#1240). Use airline_code (not the
