 ...
 * code when AirTrail doesn't provide a name. Used for what Tripp displays/stores; the
 ...
 * `date`; the airport-local wall time is what Tripp shows and files days by.
 */
...
export function mapFlightToReservation(raw: AirtrailFlightRaw): MappedReservation {
  // Read the SCHEDULED times only — Tripp plans against the scheduled (booked) time,
 ...
 * remote changes (AirTrail exposes no updated_at/etag) and to suppress Tripp's own
