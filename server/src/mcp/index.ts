...
You are connected to Tripp, a travel planning application. Below is a compact reference of the data model, key workflows, and behavioral rules you must follow.
...
The following features are optional and may not be available on every Tripp instance. Check tool availability before assuming they exist:
...
const STATIC_TOKEN_DEPRECATION_NOTICE =
    ...
    '"Your Tripp integration is using a static API token that is deprecated and will stop working in a future version. ' +
    'Please migrate to OAuth 2.1: go to Settings → Integrations → MCP → OAuth Clients in Tripp and register an OAuth 2.1 application." ' +
...
function setAuthChallenge(res: Response, error = 'invalid_token'): void {
  ...
  res.set('WWW-Authenticate',
      `Bearer realm="Tripp MCP", resource_metadata="${base}/.well-known/oauth-protected-resource/mcp", error="${error}"`);
}
...
function verifyToken(authHeader: string | undefined): VerifyTokenResult | null {
  ...
  // Short-lived JWT (Tripp web session used directly) — full access, no notice
