...
const tr: NotificationLocale = {
  email: {
    footer: "Tripp'te bildirimleri etkinleştirdiğiniz için bunu aldınız.",
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için Tripp'i açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni Tripp sürümü mevcut',
      body: `Tripp ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'Tripp hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
