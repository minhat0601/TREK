...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in Tripp.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open Tripp to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New Tripp version available',
      body: `Tripp ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your Tripp account. Click the button below to set a new password.',
