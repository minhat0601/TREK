...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in Tripp.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri Tripp per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione Tripp disponibile',
      body: `Tripp ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account Tripp. Clicca il pulsante qui sotto per impostare una nuova password.',
