...
const ko: NotificationLocale = {
  email: {
    footer: 'Tripp에서 알림을 활성화했기 때문에 이 이메일을 받으셨습니다.',
    manage: '설정에서 환경설정 관리',
    madeWith: 'Made with',
    openTrek: 'Tripp 열기',
  },
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}이(가) 휴가 계획을 합치도록 초대했습니다. Tripp을 열어 수락하거나 거절하세요.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '새 Tripp 버전 사용 가능',
      body: `Tripp ${p.version}을 사용할 수 있습니다. 관리자 패널에서 업데이트하세요.`,
    }),
  ...
  passwordReset: {
    subject: '비밀번호 재설정',
    greeting: '안녕하세요',
    body: 'Tripp 계정 비밀번호 재설정 요청을 받았습니다. 아래 버튼을 클릭하여 새 비밀번호를 설정하세요.',
    ctaIntro: '비밀번호 재설정',
