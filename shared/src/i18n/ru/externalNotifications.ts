...
const ru: NotificationLocale = {
  email: {
    footer: 'Вы получили это, потому что у вас включены уведомления в Tripp.',
    manage: 'Управление настройками',
    madeWith: 'Made with',
    openTrek: 'Открыть Tripp',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} приглашает вас объединить планы отпуска. Откройте Tripp для подтверждения.`,
    }),
    photos_shared: (p) => ({
      title: `${p.count} фото`,
    ...
    version_available: (p) => ({
      title: 'Доступна новая версия Tripp',
      body: `Tripp ${p.version} теперь доступен. Перейдите в панель администратора для обновления.`,
    }),
  ...
  passwordReset: {
    ...
    body: 'Мы получили запрос на сброс пароля вашего аккаунта Tripp. Нажмите кнопку ниже, чтобы установить новый пароль.',
