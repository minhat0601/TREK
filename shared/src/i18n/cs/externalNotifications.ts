...
const cs: NotificationLocale = {
  email: {
    footer: 'Toto jsi obdržel/a, protože máš povoleny upozornění v Tripp.',
    manage: 'Spravovat předvolby v nastavení',
    madeWith: 'Made with',
    openTrek: 'Otevřít Tripp',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete Tripp pro přijetí nebo odmítnutí.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nová verze Tripp dostupná',
      body: `Tripp ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ahoj',
    body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu Tripp. Klikni na tlačítko níže a nastav nové heslo.',
