...
const pl: NotificationLocale = {
  email: {
    footer: 'Otrzymałeś/aś tę wiadomość, ponieważ masz włączone powiadomienia w Tripp.',
    manage: 'Zarządzaj preferencjami w ustawieniach',
    madeWith: 'Made with',
    openTrek: 'Otwórz Tripp',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz Tripp, aby zaakceptować lub odrzucić.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nowa wersja Tripp dostępna',
      body: `Tripp ${p.version} jest teraz dostępny. Odwiedź panel administracyjny, aby zaktualizować.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Cześć',
    body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta Tripp. Kliknij przycisk poniżej, aby ustawić nowe hasło.',
