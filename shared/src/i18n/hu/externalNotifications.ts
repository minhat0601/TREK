...
const hu: NotificationLocale = {
  email: {
    footer: 'Ezt az értesítést azért kaptad, mert engedélyezted az értesítéseket a Tripp-ben.',
    manage: 'Beállítások kezelése',
    madeWith: 'Made with',
    openTrek: 'Tripp megnyitása',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a Tripp-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Új Tripp verzió érhető el',
      body: `A Tripp ${p.version} elérhető. Látogasson el az adminisztrációs panelre a frissítéshez.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Szia',
    body: 'Kérést kaptunk a Tripp-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.',
