...
const nl: NotificationLocale = {
  email: {
    footer: 'Je ontvangt dit omdat je meldingen hebt ingeschakeld in Tripp.',
    manage: 'Voorkeuren beheren',
    madeWith: 'Made with',
    openTrek: 'Tripp openen',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} nodigt je uit om vakantieplannen te fuseren. Open Tripp om te accepteren of af te wijzen.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nieuwe Tripp-versie beschikbaar',
      body: `Tripp ${p.version} is nu beschikbaar. Bezoek het beheerderspaneel om bij te werken.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hallo',
    body: 'We hebben een verzoek ontvangen om het wachtwoord voor je Tripp-account te resetten. Klik op de knop hieronder om een nieuw wachtwoord in te stellen.',
