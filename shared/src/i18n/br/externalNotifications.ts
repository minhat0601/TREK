...
const br: NotificationLocale = {
  email: {
    footer: 'Você recebeu isso porque tem as notificações ativadas no Tripp.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} convidou você para fundir planos de férias. Abra o Tripp para aceitar ou recusar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nova versão do Tripp disponível',
      body: `O Tripp ${p.version} está disponível. Acesse o painel de administração para atualizar.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Olá',
    body: 'Recebemos um pedido para redefinir a senha da sua conta Tripp. Clique no botão abaixo para definir uma nova senha.',
