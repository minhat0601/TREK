...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i Tripp.',
    manage: 'Hantera egenskaper under Inställningar',
    madeWith: 'Gjorde med',
    openTrek: 'Öppna Tripp',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna Tripp för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny Tripp version tillgänglig',
      body: `Tripp ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt Tripp konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
