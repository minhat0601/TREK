...
const gr: NotificationLocale = {
  email: {
    footer: 'Λάβατε αυτό το μήνυμα επειδή έχετε ενεργοποιήσει τις ειδοποιήσεις στο Tripp.',
    manage: 'Διαχείριση προτιμήσεων στις Ρυθμίσεις',
    madeWith: 'Δημιουργήθηκε με',
    openTrek: 'Άνοιγμα Tripp',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `Ο/Η ${p.actor} σας προσκάλεσε να συγχωνεύσετε τα σχέδια διακοπών σας. Ανοίξτε το Tripp για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Νέα έκδοση Tripp διαθέσιμη',
      body: `Η έκδοση Tripp ${p.version} είναι τώρα διαθέσιμη. Επισκεφθείτε τον πίνακα διαχείρισης για να ενημερώσετε.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Γεια σας',
    body: 'Λάβαμε ένα αίτημα επαναφοράς του κωδικού πρόσβασης για τον λογαριασμό σας στο Tripp. Κάντε κλικ στο παρακάτω κουμπί για να ορίσετε νέο κωδικό πρόσβασης.',
