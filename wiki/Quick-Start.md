...
Get Tripp running in under five minutes with a single Docker command.
...
**Why the encryption key?** Tripp encrypts stored secrets (API keys, MFA seeds, OIDC credentials) using this key. If you skip it, Tripp auto-generates one and saves it to `./data/.encryption_key`. Setting it explicitly means you control the key and can back it up separately.
...
On first boot Tripp automatically seeds an admin account before any user registers. The credentials depend on how you start the container:

- **With `ADMIN_EMAIL` and `ADMIN_PASSWORD` env vars set:** those values are used directly.
- **Without those env vars:** Tripp creates the account with email `admin@trek.local`, username `admin`, and a randomly generated password. The credentials are printed to the container log — run `docker logs trek` to retrieve them.
...
- [Reverse-Proxy](Reverse-Proxy) — put Tripp behind HTTPS (required for PWA install and secure cookies)
