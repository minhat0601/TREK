...
Install Tripp on Unraid via Community Applications or a direct template import.
...
2. Search for **Tripp**.
3. Click **Install** on the Tripp result.
...
| `ADMIN_PASSWORD` | *(empty)* | Password for the first admin account (first-boot only). Must be set together with `ADMIN_EMAIL`. If either is omitted, Tripp creates the account with email `admin@trek.local` and a random password printed to the container log. |
...
Copy the output into the `ENCRYPTION_KEY` field before starting the container for the first time. If you skip this, Tripp auto-generates a key and saves it to `data/.encryption_key` — your data is still protected, but you should record that file in your backups.
...
On first boot, Tripp automatically creates an admin account. The credentials are printed to the container log — check **Docker → trek → Log** in the Unraid UI. If you set both `ADMIN_EMAIL` and `ADMIN_PASSWORD`, those values are used; otherwise the email is `admin@trek.local` and a random password is generated.
